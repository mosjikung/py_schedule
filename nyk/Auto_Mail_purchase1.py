...
def ExPortQnNoColor():
  P_ActiveQ1 = 0
  P_ActiveQ2 = 0
  FileNameXLSX = 'WARNING_REPORT_OPENPO'+str(P_Time1)+'.xlsx'
  ...
  cursor = conn.cursor()
  cursor.execute(""" SELECT WEEKMSAPPROVED,CREATE_DATE,MS_APPROVED_DATE,RS_NO,YARN_ITEM,YARN_COLOR,ITEM_FABRIC,END_BUYER,QUANTITY,UOM_CODE,NEED_BY_DATE,ORDERED_DATE
 FROM RSPR_SUMMARY_VIEW """)
  ...
  fullpath = r'C:\GITProject\pyschedule\nyk\File\WARNING_REPORT_OPENPO'+str(P_Time1)+'.xlsx'
  wb.save(fullpath)
...
def SendMailQN():
  ...
  conn = cx_Oracle.connect(user="SF5", password="OMSF5",dsn=my_dsn, encoding="UTF-8", nencoding="UTF-8")
  cursor = conn.cursor()
  cursor.execute(""" Select Distinct 'A',SUPPORT_D2 From DFIT_SUPPORT Where SUPPORT_GRP='SCR' AND SUPPORT_ID='AUTO' AND SUPPORT_D1='MAIL'""")
  ...
  msg = MIMEMultipart()
  message = "\r\r\n Thank you"
  body = "To. All \n \nThis is an Auto-email alert pending RS & PR \nPlease see information in attach file.\n "
  ...
  msg['To'] = P_REC
  msg['Subject'] = 'Auto-email alert pending RS & PR'
    
    # att = MIMEApplication(open("QN-Data-NoColor.xlsx", "rb").read())
    # msg.attach(att)
    # msg.attach(MIMEText(message, 'plain'))

  filename = r'WARNING_REPORT_OPENPO'+str(P_Time1)+'.xlsx'
  attachment = open(r'C:\GITProject\pyschedule\nyk\File\WARNING_REPORT_OPENPO'+str(P_Time1)+'.xlsx', 'rb')
