...
def AUTO_ALERT_APPPOALL():
 try:
  ...
  conn = cx_Oracle.connect(user="nygm", password="nygm", dsn=my_dsn)
     ...
     try:    
        cursor = conn.cursor()
        sendLine("Auto email notify PO Approved  on soft square system")
        cursor.callproc('NYG_Daily_Auto_Alert_POAPP_F')
       
     except Exception  as err:
        print('Can not run Procedures',err)
        sendLine("Can not run Procedures Error",err)
     else:
        print('Run Procedures  Auto email notify PO Approved  on soft square system')
        sendLine("Run Procedures  Auto email notify PO Approved  on soft square system")
     finally:
        cursor.close()
 finally:
     conn.close()
