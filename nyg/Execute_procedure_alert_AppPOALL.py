...
def AUTO_ALERT_APPPOALL():
 try:
  ...
  conn = cx_Oracle.connect(user="nygm", password="nygm", dsn=my_dsn)
     ...
     try:    
        cursor = conn.cursor()
        sendLine("Start Procerdures Auto email notify  PO waiting for approve  all BU")
        cursor.callproc('NYG_Daily_Auto_Alert_PO_AppAll')
       
     except Exception  as err:
        print('Can not run Procedures',err)
        sendLine("Can not run Procedures Error",err)
     else:
        print('Run Procedures  Auto email notify  PO waiting for approve  all BU')
        sendLine("Run Procedures  Auto email notify  PO waiting for approve  all BU")
     finally:
        cursor.close()
 finally:
     conn.close()
