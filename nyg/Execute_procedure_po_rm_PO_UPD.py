...
def GET_PO_RM_PO_UPD():
 try:
  ...
  conn = cx_Oracle.connect(user="nygm", password="nygm", dsn=my_dsn)
     ...
     try:    
        cursor = conn.cursor()
        print('Start Run  Procerdures AUTO_UPD_NY_PO_NYG_PO_UPD  ')
        sendLine("Start Procerdures AUTO_UPD_NY_PO_NYG_PO_UPD")
        cursor.callproc('AUTO_UPD_NY_PO_NYG_PO_UPD')            
     except Exception  as err:
        print('Can not run Procedures1',err)
        sendLine("Can not run Procedures Error",err)
     else:
        print('Run  Procerdures AUTO_UPD_NY_PO_NYG_PO_UPD Done')
        sendLine("Run Procedures  AUTO_UPD_NY_PO_NYG_PO_UPD Done")
     finally:
        cursor.close()
 finally:
     conn.close()
